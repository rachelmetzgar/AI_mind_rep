# Experiment 5 — Mental-State Concept Deployment During Theory of Mind Reasoning
...
Exp 5 bridges these approaches: **does the model activate mental-state concept representations (awareness, attention, consciousness) in a context-sensitive way that tracks characters' knowledge states during theory of mind tasks?**
...
|---|---|---|
| Exp 1 | LLMs adjust behavior based on partner identity labels | Exp 5 tests whether mental-state concepts are deployed during social reasoning, not just identity-conditioned behavior |
| Exp 2 | Partner identity is linearly decodable and causally active | Exp 5 tests whether the same representational space is used during ToM tasks |
| Exp 3 | LLMs have concept-level representations of mental properties | Exp 5 tests whether those representations are *used* during reasoning, not just *present* |
| Exp 4 | LLMs have implicit mind perception geometry | Exp 5 tests dynamic deployment of mind concepts, not static entity ratings |
| Wu et al. | ToM depends on positional encoding parameters | Exp 5 tests whether mental-state representations ride on top of that positional structure |
