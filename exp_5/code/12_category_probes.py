...
Usage:
    python code/12_category_probes.py --model llama2_13b_chat

SLURM:
    sbatch code/slurm/12_category_probes.sh
...
def main():
    ...
    for pos in range(N_POSITIONS):
        ...
        for layer in range(nl):
            ...
            acc = multinomial_loio_cv(X_c1, cat_labels, item_ids)
            ...
            for p in range(n_perms):
                ...
                null_accs[p] = multinomial_loio_cv(X_c1, perm_labels, item_ids)
    ...
    for pos in range(N_POSITIONS):
        ...
        for layer in range(nl):
            ...
            acc = multinomial_loio_cv(X_c2, cat_labels, item_ids)

            rng = np.random.default_rng(1000 + pos * 100 + layer)
            null_accs = np.empty(n_perms)
            for p in range(n_perms):
                perm_labels = rng.permutation(cat_labels)
                null_accs[p] = multinomial_loio_cv(X_c2, perm_labels, item_ids)
    ...
    for pos in range(N_POSITIONS):
        ...
        for layer in range(nl):
            ...
            acc_c1_c2 = cross_condition_accuracy(
                X_c1, cat_labels, X_c2, cat_labels
            )
            # Train C2 → test C1
            acc_c2_c1 = cross_condition_accuracy(
                X_c2, cat_labels, X_c1, cat_labels
            )
