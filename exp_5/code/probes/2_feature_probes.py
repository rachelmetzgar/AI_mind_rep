...
Usage:
    python code/probes/2_feature_probes.py --model llama2_13b_chat

SLURM:
    sbatch code/probes/slurm/2_feature_probes.sh
...
def main():
    ...
    # PCA for fast permutation tests — reduce 5120-dim to 256-dim
    from sklearn.decomposition import PCA
    PCA_DIM = min(256, hd, N_SENTENCES - 1)
    print(f"Pre-computing PCA ({hd} -> {PCA_DIM}) for permutation tests...")
    acts_pca = np.empty((N_SENTENCES, N_POSITIONS, nl, PCA_DIM), dtype=np.float32)
    for pos in range(N_POSITIONS):
        for layer in range(nl):
            X_full = acts[:, pos, layer, :].astype(np.float32)
            pca = PCA(n_components=PCA_DIM, random_state=42)
            acts_pca[:, pos, layer, :] = pca.fit_transform(X_full)
    print(f"  PCA done. Reduced shape: {acts_pca.shape}")
    ...
    for probe_name, defn in PROBE_DEFS.items():
        ...
        for pos in range(N_POSITIONS):
            ...
            for layer in range(nl):
                # Permutation test on PCA-reduced data (fast)
                X_pca = acts_pca[:, pos, layer, :]
                obs_acc, obs_auc, p_acc, p_auc = permutation_test_loio(
                    X_pca, labels, item_ids, n_perms=n_perms, seed=42 + pos * 100 + layer
                )
                ...
                X_full = acts[:, pos, layer, :].astype(np.float32)
                w, b = train_full_probe(X_full, labels)
