...
def _has_bos(input_ids, tokenizer):
    """Check whether the tokenizer prepended a BOS token."""
    return tokenizer.bos_token_id is not None and input_ids[0] == tokenizer.bos_token_id


def _find_span_by_decode(input_ids, target_str, tokenizer, search_start=None):
    """Find token span whose decoded text matches target_str.

    Tokenizer-agnostic: works with sentencepiece (LLaMA-2), BPE (LLaMA-3),
    and tokenizers without BOS (Qwen) by decoding candidate spans and
    comparing strings.
    ...
    Args:
        ...
        search_start: first token index to consider. If None, auto-detects
                      (skip BOS if present, otherwise start at 0).
    ...
    if search_start is None:
        search_start = 1 if _has_bos(input_ids, tokenizer) else 0
