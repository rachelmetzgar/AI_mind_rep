...
def permutation_test_loio(X, y, item_ids, n_perms=200, seed=42,
    ...
    Args:
        ...
        seed: random seed
        two_sided: if True, test |AUC - 0.5| >= |obs_AUC - 0.5| (detects
            both above- and below-chance discrimination). Use for residual
            probes where deconfounding can invert the signal.
        **kwargs: forwarded to loio_accuracy_auc

    Returns:
        observed_acc: float
        observed_auc: float
        p_acc: float — proportion of null >= observed accuracy (or two-sided)
        p_auc: float — proportion of null >= observed AUC (or two-sided)
    """
    ...
    if two_sided:
        # Test deviation from chance in either direction
        obs_dev_auc = abs(obs_auc - 0.5)
        null_dev_auc = np.abs(null_aucs - 0.5)
        p_auc = float(np.mean(null_dev_auc >= obs_dev_auc))

        # For accuracy, chance depends on class balance — use 0.75 for 1:3 ratio,
        # but safer to use the null mean as the expected value
        null_mean_acc = np.mean(null_accs)
        obs_dev_acc = abs(obs_acc - null_mean_acc)
        null_dev_acc = np.abs(null_accs - null_mean_acc)
        p_acc = float(np.mean(null_dev_acc >= obs_dev_acc))
    else:
