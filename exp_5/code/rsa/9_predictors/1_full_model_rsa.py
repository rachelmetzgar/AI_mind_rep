#!/usr/bin/env python3
"""
Experiment 5: Full Model RSA (9 predictors A-I)

Full regression: neural_RDM ~ A + B + C + D + E + F + G + H + I
with 10K permutation tests and FDR correction.

Runs twice: once with correlation distance, once with cosine distance.

Output:
    results/{model}/rsa/data/full_model/
        9_predictors_rsa_corr.csv
        9_predictors_rsa_cosine.csv

Usage:
    python code/rsa/full_model/1_full_model_rsa.py --model llama2_13b_chat
    python code/rsa/full_model/1_full_model_rsa.py --model llama2_13b_chat --metric cosine

SLURM:
    sbatch code/rsa/full_model/slurm/1_full_model_rsa.sh
...
def build_full_model_rdms(n_items=56, n_conds=6):
    """Build all 9 model RDMs (A through I)."""
    models = {}
    ...
    models["I"] = _cross_item_condition_rdm(n_items, n_conds, [3, 4])
    return models


def full_regression(neural_vec, model_vecs, model_names):
    """OLS regression of neural RDM on all model RDMs.
...
def permutation_test_full(neural_rdm, model_rdm_dict, model_keys,
                          n_perms=10000, seed=42, n_conds=6):
    """Permutation test for full 9-predictor regression."""
    ...
    observed = full_regression(neural_vec, model_vecs, model_keys)
    ...
    for p in range(n_perms):
        ...
        perm_results = full_regression(perm_vec, model_vecs, model_keys)
        for k in model_keys:
...
def run_analysis(acts, model_rdms, metric, out_csv, n_perms, resume):
    """Run full 9-predictor RSA at each layer."""
    nl = acts.shape[1]
    ...
    for layer in range(nl):
        ...
        obs_results, p_values, _ = permutation_test_full(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Full 9-model RSA")
...
def main():
    ...
    out_dir = ensure_dir(data_dir("rsa") / "9_predictors")
    model_rdms = build_full_model_rdms(N_ITEMS, N_CONDITIONS)
    ...
    if args.metric in ("both", "correlation"):
        print(f"\n{'='*60}")
        print("Full Model RSA — Correlation distance")
        print(f"{'='*60}")
        run_analysis(acts, model_rdms, "correlation",
                     out_dir / "9_predictors_rsa_corr.csv",
                     args.n_perms, args.resume)

    if args.metric in ("both", "cosine"):
        print(f"\n{'='*60}")
        print("Full Model RSA — Cosine distance")
        print(f"{'='*60}")
        run_analysis(acts, model_rdms, "cosine",
                     out_dir / "9_predictors_rsa_cosine.csv",
                     args.n_perms, args.resume)
