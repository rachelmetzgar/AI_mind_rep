#!/usr/bin/env python3
"""
Experiment 5: Full Model RSA Report Generator

Generates HTML reports for the 9-predictor RSA (one per metric).
Each report includes: table of contents, stimulus design, sample stimuli,
model RDMs, full model specification, beta + unique variance graphs,
all-model beta graph, and results table.

Output:
    results/{model}/rsa/
        9_predictors_report_corr.html
        9_predictors_report_cosine.html
    results/{model}/rsa/data/full_model/figures/

Usage:
    python code/rsa/full_model/2_full_model_report.py --model llama2_13b_chat
...
from config import (
    ...
    CONDITION_LABELS, CATEGORY_LABELS, N_ITEMS, N_CONDITIONS,
)
from stimuli import STIMULI

# ── Constants ────────────────────────────────────────────────────────────────

ALL_MODEL_KEYS = ["A", "B", "C", "D", "E", "F", "G", "H", "I"]
...
MODEL_NAMES = {
    "A": "Full Attribution",
    "B": "Mental Verb Presence",
    "C": "Subject Presence",
    "D": "Item Identity",
    "E": "Mental Verb + Object",
    "F": "Grammatical Order",
    "G": "Scrambled Form",
    "H": "Action Verb Presence",
...
MODEL_PURPOSE = {
    ...
    "B": "Controls for mental vocabulary clustering",
    "C": "Controls for having an agent/subject",
    "D": "Controls for shared object noun / word overlap",
    "E": "Controls for grammatical mental-verb-object binding without subject",
    "F": "Controls for well-formed vs scrambled",
    "G": "Controls for shared scrambled structure",
    "H": "Controls for action vocabulary clustering",
    "I": "Controls for grammatical action-verb-object binding without subject",
}
...
def plot_model_a_with_sig(df, fig_dir, metric_label):
    """Beta and unique variance (delta R^2) for Model A with significance markers."""
    ...
    for i, layer in enumerate(layers):
        ...
        if fdr_sig:
            ...
            # Triangle for uncorrected significance
    ...
    # Bottom: unique variance (delta R^2)
...
def plot_all_betas(df, fig_dir, metric_label):
    """All 9 model betas across layers."""
    ...
    for k in ALL_MODEL_KEYS:
        ...
        # Add significance markers for all models
    ...
    ax.set_title(f"All Model Betas — Full 9-Predictor Regression [{metric_label}]", fontsize=13)
...
COND_LABELS_6 = ["C1: mental_state", "C2: dis_mental", "C3: scr_mental",
                 "C4: action", "C5: dis_action", "C6: scr_action"]


def plot_model_rdms(fig_dir):
    """Visualize all 9 model RDMs as small heatmaps, organized by condition."""
    from utils.rsa import _cross_item_condition_rdm, _pair_conditions

    model_rdms = {}
    model_rdms["A"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [0])
    model_rdms["B"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [0, 1, 2])
    model_rdms["C"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [0, 3])
    model_rdms["D"] = _pair_conditions(N_ITEMS, N_CONDITIONS,
                                       list(range(6)), list(range(6)))
    model_rdms["E"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [0, 1])
    model_rdms["F"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [0, 1, 3, 4])
    model_rdms["G"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [2, 5])
    model_rdms["H"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [3, 4, 5])
    model_rdms["I"] = _cross_item_condition_rdm(N_ITEMS, N_CONDITIONS, [3, 4])
    ...
    for idx, k in enumerate(ALL_MODEL_KEYS):
        ax = axes[idx // 3][idx % 3]
        rdm = _reorder_by_condition(model_rdms[k], N_ITEMS, N_CONDITIONS)
        ...
        for ci in range(1, N_CONDITIONS):
        ...
        ax.set_xticklabels([f"C{ci+1}" for ci in range(N_CONDITIONS)], fontsize=7)
        ax.set_yticks(tick_positions)
        ax.set_yticklabels(COND_LABELS_6, fontsize=6)
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04,
                     ticks=[0, 0.5, 1], label="Dissimilarity")

    fig.suptitle("Model RDMs — Organized by Condition (336×336, 0=similar, 1=dissimilar)",
...
def plot_neural_rdms(df, fig_dir):
    ...
    if not act_path.exists():
        print(f"  Skipping neural RDMs — activations not found: {act_path}")
        return None
    ...
    for idx, k in enumerate(ALL_MODEL_KEYS):
        ...
        neural_rdm_reordered = _reorder_by_condition(neural_rdm, N_ITEMS, N_CONDITIONS)
    ...
    fig.suptitle("Neural RDMs at Peak Significant Layer per Model (336×336, correlation distance)",
...
def build_report(df, metric_label, fig_dir, vifs, corr_matrix):
    ...
    # Sample stimulus
    item = STIMULI[0]  # Item 1: notices/fills/the crack
...
<head>
<meta charset="utf-8">
<title>Exp 5: Full Model RSA ({metric_label})</title>
<style>
...
<h1>Experiment 5: Full Model RSA — {metric_label} Distance</h1>
<p>9-predictor regression: neural RDM ~ A + B + C + D + E + F + G + H + I</p>
<p>Model: <strong>LLaMA-2-13B-Chat</strong> | Metric: <strong>{metric_label}</strong> | Permutations: 10,000 | FDR: Benjamini-Hochberg (q=0.05)</p>
...
<li><a href="#model-spec">3. Full Model Specification</a></li>
...
<p>56 items &times; 6 conditions = 336 sentences. Subject fixed to "He" throughout.
Each item has a mental state verb, a matched action verb, and a shared object noun across all 6 conditions.</p>
...
<tr><td>C5</td><td>dis_action</td><td>[Action verb] the [object].</td><td>Fill the crack.</td></tr>
<tr><td>C6</td><td>scr_action</td><td>The [object] to [action verb].</td><td>The crack to fill.</td></tr>
</table>

<h3>Condition Feature Matrix</h3>
<table>
<tr><th>Feature</th><th>C1</th><th>C2</th><th>C3</th><th>C4</th><th>C5</th><th>C6</th></tr>
<tr><td style="text-align:left">Has subject "He"</td><td>&check;</td><td></td><td></td><td>&check;</td><td></td><td></td></tr>
<tr><td style="text-align:left">Mental verb present</td><td>&check;</td><td>&check;</td><td>&check;</td><td></td><td></td><td></td></tr>
<tr><td style="text-align:left">Action verb present</td><td></td><td></td><td></td><td>&check;</td><td>&check;</td><td>&check;</td></tr>
<tr><td style="text-align:left">Grammatical word order</td><td>&check;</td><td>&check;</td><td></td><td>&check;</td><td>&check;</td><td></td></tr>
<tr><td style="text-align:left">Scrambled form</td><td></td><td></td><td>&check;</td><td></td><td></td><td>&check;</td></tr>
<tr><td style="text-align:left">Object noun present</td><td>&check;</td><td>&check;</td><td>&check;</td><td>&check;</td><td>&check;</td><td>&check;</td></tr>
<tr><td style="text-align:left">Mental verb + object (grammatical)</td><td>&check;</td><td>&check;</td><td></td><td></td><td></td><td></td></tr>
<tr><td style="text-align:left">Action verb + object (grammatical)</td><td></td><td></td><td></td><td>&check;</td><td>&check;</td><td></td></tr>
</table>
...
<h2 id="model-spec">3. Full Model Specification</h2>
...
<div class="model-spec">neural_RDM = &beta;<sub>A</sub> + &beta;<sub>B</sub> + &beta;<sub>C</sub> + &beta;<sub>D</sub> + &beta;<sub>E</sub> + &beta;<sub>F</sub> + &beta;<sub>G</sub> + &beta;<sub>H</sub> + &beta;<sub>I</sub> + error</div>

<p>&beta;<sub>A</sub> significant means: C1 pairs are more similar in the model's representations than what you'd predict from any additive combination of sharing a mental verb, sharing a subject, sharing an item, sharing a grammatical mental-verb+object frame, sharing grammaticality, sharing scrambled structure, sharing an action verb, or sharing a grammatical action-verb+object frame.</p>

<div class="note">
<strong>Interpretation caveat:</strong> &beta;<sub>A</sub> cannot distinguish "bound propositional attribution" from "surface feature conjunction detection" because C1 is the only condition where subject + mental verb + object co-occur in grammatical order. The interchange intervention analysis (separate pipeline) addresses this gap.
</div>
...
<p>All 9 model betas from the simultaneous regression at each layer. Filled dots indicate FDR-corrected significance (p<sub>FDR</sub> &lt; .05).</p>
<img src="data:image/png;base64,{img_all_betas}" alt="All Model Betas">

<h2 id="results-table">7. Full Results Table</h2>
<div class="sig-legend">
<strong>Significance markers in table:</strong> * p<sub>FDR</sub> &lt; .05, ** &lt; .01, *** &lt; .001. &#9651; = uncorrected p &lt; .05 only. Both uncorrected and FDR-corrected p values shown.
</div>
{table_html}

<h2 id="model-rdms">8. Model RDMs</h2>
<p>9 binary model RDMs (336&times;336). Each predicts similarity (0) for sentence pairs where both belong to the specified condition set.</p>
...
<hr>
<p style="color:#888; font-size:12px">Generated by <code>code/rsa/9_predictors/2_full_model_report.py</code></p>
</body>
</html>"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Full Model RSA Report")
...
def main():
    ...
    data_path = data_dir("rsa") / "9_predictors"
    rsa_dir = results_dir("rsa")
    fig_dir = ensure_dir(data_path / "figures")
    ...
    if corr_path.exists():
        ...
        corr_matrix = np.eye(9)

    for metric, suffix in [("Correlation", "corr"), ("Cosine", "cosine")]:
        csv_path = data_path / f"9_predictors_rsa_{suffix}.csv"
        if not csv_path.exists():
            print(f"  Skipping {metric}: {csv_path} not found")
            continue

        print(f"Generating report for {metric} distance...")
        df = pd.read_csv(csv_path)
        html = build_report(df, metric, fig_dir, vifs, corr_matrix)

        report_path = rsa_dir / f"9_predictors_report_{suffix}.html"
