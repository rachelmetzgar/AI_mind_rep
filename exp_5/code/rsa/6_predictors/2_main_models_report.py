#!/usr/bin/env python3
"""
Experiment 5: Main Models RSA Report Generator

Generates HTML reports for the 6-predictor RSA (A-F, one per metric).
...
Output:
    results/{model}/rsa/
        6_predictors_report_corr.html
        6_predictors_report_cosine.html
    results/{model}/rsa/data/main_models/figures/

Usage:
    python code/rsa/main_models/2_main_models_report.py --model llama2_13b_chat
...
def plot_all_betas(df, fig_dir, metric_label):
    """All 6 model betas across layers."""
    ...
    ax.set_title(f"All Model Betas — 6-Predictor Regression [{metric_label}]", fontsize=13)
...
def plot_model_rdms(fig_dir):
    """Visualize all 6 model RDMs as small heatmaps, organized by condition."""
...
def build_report(df, metric_label, fig_dir, vifs, corr_matrix):
    ...
    beta_interp = ("&beta;<sub>A</sub> significant means: C1 pairs are more similar in the model's "
                   "representations than what you'd predict from any additive combination of sharing a "
                   "mental verb (B), sharing a subject (C), sharing an item (D), sharing a grammatical "
                   "mental-verb+object frame (E), or sharing grammatical order (F).")

    html = f"""<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>Exp 5: Main Models RSA ({metric_label})</title>
<style>
...
<h1>Experiment 5: Main Models RSA — {metric_label} Distance</h1>
<p>6-predictor regression: neural RDM ~ {beta_formula} + error</p>
...
<p>All 6 model betas from the simultaneous regression at each layer. Filled dots indicate FDR-corrected significance (p<sub>FDR</sub> &lt; .05).</p>
<img src="data:image/png;base64,{img_all_betas}" alt="All Model Betas">

<h2 id="results-table">7. Full Results Table</h2>
<div class="sig-legend">
<strong>Significance markers in table:</strong> * p<sub>FDR</sub> &lt; .05, ** &lt; .01, *** &lt; .001. &triangle; = uncorrected p &lt; .05 only. Both uncorrected and FDR-corrected p values shown.
</div>
{table_html}

<h2 id="model-rdms">8. Model RDMs</h2>
<p>6 binary model RDMs (336&times;336). Each predicts similarity (0) for sentence pairs where both belong to the specified condition set.</p>
...
<hr>
<p style="color:#888; font-size:12px">Generated by <code>code/rsa/6_predictors/2_main_models_report.py</code></p>
</body>
</html>"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Main Models RSA Report")
...
def main():
    ...
    data_path = data_dir("rsa") / "6_predictors"
    ...
    for metric, suffix in [("Correlation", "corr"), ("Cosine", "cosine")]:
        csv_path = data_path / f"6_predictors_rsa_{suffix}.csv"
        ...
        report_path = rsa_dir / f"6_predictors_report_{suffix}.html"
