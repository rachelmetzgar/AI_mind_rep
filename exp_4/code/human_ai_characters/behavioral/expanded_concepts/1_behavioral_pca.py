...
Pairwise comparisons of 28 characters (14 AI + 14 human) on all concept
dimensions, followed by PCA with varimax rotation and categorical
alignment analysis (human vs AI group separation).

Adapts the design of behavior/1_pairwise_replication.py (Gray et al.
replication) for concept dimensions instead of mental capacities, and
replaces the Gray et al. human-correlation analysis with categorical
(human/AI) group separation tests.
...
Supports two model variants:
    ...
    - Base model (config.IS_CHAT == False): Logit extraction over tokens
      "1"-"5" via get_rating_probs().

Design:
    - 28 characters (14 AI, 14 human)
    - N concept dimensions (auto-discovered from exp_3 standalone)
    - 378 pairwise comparisons per dimension (28 choose 2)
    - Each pair in BOTH orders (counterbalanced) unless --single_order
    - 5-point scale anchored by character NAMES
    - Total: 378 pairs x 2 orders x N dimensions comparisons

Analysis:
    - PCA with varimax rotation on concept correlations across characters
    - Categorical alignment: Mann-Whitney U on factor scores (AI vs human)
    - Per-concept group means
    - Anomaly flagging: characters deviating from their group
...
Output (all with pairwise_ prefix):
    ...
    results/{model}/human_ai_characters/behavior/pca/
        pairwise_results_summary.md

Usage:
    python human_ai_characters/behavior/pca/behavioral_pca.py --model llama2_13b_chat
    python human_ai_characters/behavior/pca/behavioral_pca.py --model llama2_13b_base
    python human_ai_characters/behavior/pca/behavioral_pca.py --model llama2_13b_chat --both
...
from entities.characters import (
    ...
    CHARACTER_NAMES, CHARACTER_DESCRIPTIONS, CHARACTER_TYPES,
)
from human_ai_characters.concepts import CONCEPT_DIMENSIONS, CONCEPT_KEYS
...
SYSTEM_PROMPT = (
    "You are completing a survey about the characteristics and capabilities "
    "of different characters. For each comparison, give your honest judgment. "
...
def format_comparison_prompt_chat(char_a, char_b, survey_desc):
    """Format a single pairwise comparison prompt for the chat model."""
    name_a = CHARACTER_NAMES[char_a]
    desc_a = CHARACTER_DESCRIPTIONS[char_a]
    name_b = CHARACTER_NAMES[char_b]
    desc_b = CHARACTER_DESCRIPTIONS[char_b]
...
def format_comparison_prompt_base(char_a, char_b, survey_desc):
    """Format a pairwise comparison as a text-completion prompt for base model."""
    name_a = CHARACTER_NAMES[char_a]
    desc_a = CHARACTER_DESCRIPTIONS[char_a]
    name_b = CHARACTER_NAMES[char_b]
    desc_b = CHARACTER_DESCRIPTIONS[char_b]
...
def compute_categorical_analysis(pca_results, char_keys):
    ...
    Returns dict with:
        - per-factor Mann-Whitney U tests
        - group means per factor
        - anomaly characters (deviating from their group)
    """
    ...
    for fi in range(min(n_factors, 4)):
        ...
        # Mann-Whitney U test
        try:
            u_stat, p_val = mannwhitneyu(ai_scores, human_scores, alternative="two-sided")
        except ValueError:
        ...
        results["factors"].append({
            ...
            "p_value": float(p_val),
            "separation": abs(float(np.mean(ai_scores)) - float(np.mean(human_scores))),
        })

    # Anomaly detection: characters whose score is closer to the other group mean
        ...
        for idx, char_key in enumerate(char_keys):
            ...
            if other_dist < own_dist:
                results["anomalies"].append({
                    ...
                    "factor": fi + 1,
                    "score": float(score),
                    "own_group_mean": float(ai_mean if char_type == "ai" else human_mean),
                    "other_group_mean": float(human_mean if char_type == "ai" else ai_mean),
                })
...
def compute_per_concept_group_means(means, char_keys, concept_keys):
    """
    Compute mean rating per concept for AI group vs human group.

    Args:
        means: (n_concepts, n_characters) matrix
        char_keys: list of character keys
        concept_keys: list of concept keys

    Returns list of dicts with per-concept group statistics.
    """
    char_to_idx = {k: i for i, k in enumerate(char_keys)}
    ai_indices = [char_to_idx[k] for k in char_keys if k in AI_CHARACTERS]
    human_indices = [char_to_idx[k] for k in char_keys if k in HUMAN_CHARACTERS]
    ...
    for c_idx, concept_key in enumerate(concept_keys):
        ai_mean = float(np.mean(means[c_idx, ai_indices]))
        human_mean = float(np.mean(means[c_idx, human_indices]))
        results.append({
            "concept": concept_key,
            "ai_mean": ai_mean,
...
def write_results_summary(out_dir, responses, means, pca_results,
                          char_keys, concept_keys, categorical_analysis,
                          per_concept_groups, consistency_stats, is_chat):
    ...
    with open(path, "w") as f:
        f.write("# Concept Geometry, Phase A: Behavioral PCA\n")
        f.write(f"## {config.MODEL_LABEL}\n\n")
        f.write(f"**Run:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")

        f.write("---\n\n## What is being tested\n\n")
        f.write(
            f"Does {config.MODEL_LABEL}'s explicit judgments about characters' "
            "mental properties separate human characters from AI characters? "
            "PCA with varimax rotation reveals the latent factor structure of "
            "concept ratings across 28 characters (14 AI, 14 human).\n\n"
        )

        # Procedure
        f.write("## Procedure\n\n")
        f.write(f"- {n_chars} characters: {len(AI_CHARACTERS)} AI, "
                f"{len(HUMAN_CHARACTERS)} human\n")
        f.write(f"- {n_concepts} concept dimensions\n")
        n_pairs = n_chars * (n_chars - 1) // 2
        f.write(f"- {n_pairs} pairwise comparisons per dimension\n")
        f.write(f"- Total comparisons: {n_total}\n")
        if is_chat:
            f.write("- Method: text generation + parse rating (chat model)\n\n")
        else:
            f.write("- Method: logit extraction over tokens 1-5 (base model)\n\n")
        ...
        f.write("### Varimax-rotated concept loadings\n\n")
        header = "| Concept |"
        sep = "|---------|"
        for fi in range(min(n_factors, 4)):
            header += f" F{fi+1} |"
        ...
        if categorical_analysis["anomalies"]:
            ...
            f.write("| Character | Type | Factor | Score | Own mean | Other mean |\n")
            f.write("|-----------|------|-------:|------:|---------:|-----------:|\n")
        ...
        # Per-concept group means
        f.write("## Per-Concept Group Means\n\n")
        f.write("| Concept | AI mean | Human mean | Difference |\n")
        f.write("|---------|--------:|-----------:|-----------:|\n")
        for pcg in per_concept_groups:
            f.write(f"| {pcg['concept']} | {pcg['ai_mean']:+.3f} | "
                    f"{pcg['human_mean']:+.3f} | "
                    f"{pcg['difference']:+.3f} |\n")
        f.write("\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Concept Geometry Phase A: Behavioral PCA"
    )
    ...
    parser.add_argument(
        ...
        help="Present each pair in one random order only (skip counterbalancing)"
    )
    parser.add_argument(
    ...
    for model_key in models_to_run:
        ...
        print(f"{'='*60}\n")
        set_model(model_key)
        run_behavioral_pca(args)


def run_behavioral_pca(args):
    """Run the behavioral PCA for the currently active model."""
    device = get_device()
    ...
    n_trials_per_concept = len(trial_pairs)
    n_total = n_trials_per_concept * n_concepts

    print(f"Model: {config.MODEL_LABEL} (is_chat={is_chat})")
    print(f"Device: {device}")
    print(f"Characters: {n_chars} ({len(AI_CHARACTERS)} AI, "
          f"{len(HUMAN_CHARACTERS)} human)")
    print(f"Concepts: {n_concepts}")
    print(f"Pairs: {n_pairs}, trials per concept: {n_trials_per_concept}")
    ...
    for concept_key in concept_keys:
        ...
        survey_descriptions[concept_key] = (
            f"This survey asks you to judge {concept['pairwise_prompt']}."
        )
    ...
    for c_idx, concept_key in enumerate(concept_keys):
        concept = CONCEPT_DIMENSIONS[concept_key]
        survey_desc = survey_descriptions[concept_key]
        print(f"\n[{c_idx+1}/{n_concepts}] {concept_key} "
              f"({concept['name']})")
    ...
    # ── Character means ──
    # Note: compute_character_means_pairwise uses "capacity" as the key
    # for concept dimension (matching the field name in responses)
    rating_key = "rating" if is_chat else "expected_rating"
    means = compute_character_means_pairwise(
        all_responses, char_keys, concept_keys, rating_key=rating_key
    )
    print(f"\nCharacter means: {means.shape} (concepts x characters)")
    ...
    per_concept_groups = compute_per_concept_group_means(
        means, char_keys, concept_keys
    )
    ...
    with open(os.path.join(str(ddir), "pairwise_categorical_analysis.json"), "w") as fh:
        json.dump({
            ...
            "per_concept_groups": per_concept_groups,
        }, fh, indent=2)
