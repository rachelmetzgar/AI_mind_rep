...
def load_rsa(tag):
    """Load rsa_results.json from data_dir("gray_entities", "neural", tag)."""
    path = data_dir("gray_entities", "neural", tag) / "rsa_results.json"
    if not path.exists():
        return None
    with open(path) as f:
        return json.load(f)
...
def load_rdm(tag):
    """Load rdm_cosine_per_layer.npz from data_dir("gray_entities", "neural", tag)."""
    path = data_dir("gray_entities", "neural", tag) / "rdm_cosine_per_layer.npz"
    if not path.exists():
        return None
...
def generate_html(model_key, dataset="full_dataset"):
    ...
    for vname in ["combined", "experience", "agency"]:
        ...
        bar_fig_dir = str(figures_dir("gray_entities", "neural",
        ...
        if rdm_ns is not None and rsa_v_ns is not None:
            ns_fig_dir = str(figures_dir("gray_entities", "neural", "without_self"))
        ...
        if rdm_ws is not None and rsa_v_ws is not None:
            ws_fig_dir = str(figures_dir("gray_entities", "neural", "with_self"))
...
"""
    ...
    # Write report
    out_dir = results_dir("gray_entities", "neural")
