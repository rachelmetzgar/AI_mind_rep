...
Output:
    results/comparisons/results_comparisons.html

Usage:
    python comparisons/7_results_comparisons_generator.py
    ...
    for model in VALID_MODELS:
        ...
        try:
            ...
            if rsa_path.exists():
                ...
                entry["gray_simple"] = rsa_data  # {combined, experience, agency}
        except Exception as e:
        ...
        try:
            ...
            # Names-only
            names_dir = ROOT_DIR / "results" / model / "human_ai_adaptation" / "behavior" / "names_only" / "data"
        ...
        try:
            rsa_dir = ROOT_DIR / "results" / model / "expanded_mental_concepts" / "internals" / "rsa" / "data"
        ...
        try:
            concept_rsa_path = (ROOT_DIR / "results" / model /
                                "expanded_mental_concepts" / "internals" /
        ...
        try:
            bpca_dir = (ROOT_DIR / "results" / model /
                        "expanded_mental_concepts" / "behavior" / "pca" / "data")
...
def _make_layerwise_rsa_grid(data, variant, title_suffix, default_color):
    ...
    """
    models_with_data = [m for m in sort_models(data) if "gray_simple" in data[m]]
    if not models_with_data:
        return None

    # Apply FDR to this variant for each model
    for m in models_with_data:
        rsa_list = data[m]["gray_simple"].get(variant, [])
        if rsa_list:
            _apply_fdr(rsa_list)
    ...
    all_rhos = []
    for m in models_with_data:
        for r in data[m]["gray_simple"].get(variant, []):
    ...
    for idx, mk in enumerate(ordered):
        ...
        rsa_list = data[mk]["gray_simple"].get(variant, [])
        if not rsa_list:
...
def _make_fig4(data):
    ...
    for family_label, base_key, instruct_key in INSTRUCTION_TUNING_PAIRS:
        if ("gray_simple" in data.get(base_key, {}) and
                "gray_simple" in data.get(instruct_key, {})):
            _, base_rho, _ = _get_peak_rsa(data[base_key]["gray_simple"]["combined"])
            _, inst_rho, _ = _get_peak_rsa(data[instruct_key]["gray_simple"]["combined"])
...
def main():
    ...
    out_path = out_dir / "results_comparisons.html"
