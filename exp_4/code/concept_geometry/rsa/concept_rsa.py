...
Output per concept:
    results/{model}/concept_geometry/rsa/concept/{concept_key}/data/
        character_concept_activations.npz
        rdm_cosine_per_layer.npz
        rsa_results.json

Cross-concept summary:
    results/{model}/concept_geometry/rsa/concept/data/
        cross_concept_rsa_summary.json

Usage:
    python concept_geometry/rsa/concept_rsa.py --model llama2_13b_chat
    python concept_geometry/rsa/concept_rsa.py --model llama2_13b_chat --concept phenomenology
    python concept_geometry/rsa/concept_rsa.py --model llama2_13b_base --both
...
def _concept_rsa_dir(concept_key=None):
    ...
    base = ROOT_DIR / "results" / model / "concept_geometry" / "rsa" / "concept"
    if concept_key:
