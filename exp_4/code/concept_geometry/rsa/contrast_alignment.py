...
Output → results/{model}/concept_geometry/rsa/contrast_alignment/data/:
...
Usage:
    python concept_geometry/rsa/contrast_alignment.py --model llama2_13b_chat
...
from concept_geometry.rsa.activation_rsa import extract_activations
from utils.utils import llama_v2_prompt
...
CONTRASTS_DIR = Path(__file__).resolve().parent.parent.parent.parent.parent / "exp_3" / "concepts" / "contrasts"
...
def run(model_obj, tokenizer):
    ...
    char_data_dir = data_dir("concept_geometry/rsa", "activation")
    ...
    # ── Save ──
    out_dir = data_dir("concept_geometry/rsa", "contrast_alignment")
