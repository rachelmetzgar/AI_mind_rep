...
Uses Gray et al.'s 18 mental capacity questions with the 30 AI/human
characters from concept_geometry (instead of Gray's original 13 entities).
This tests whether the Experience/Agency factor structure emerges when
the model rates well-known AI systems vs naturalistic human characters.
...
Design:
    ...
    - Same prompt format as 1_pairwise_replication.py
...
Output:
    ...
    results/{model}/behavior/gray_characters/
        pairwise_results_summary.md

Usage:
    python behavior/4_gray_with_characters.py --model llama2_13b_chat
    python behavior/4_gray_with_characters.py --model llama2_13b_base
    python behavior/4_gray_with_characters.py --model llama2_13b_chat --both
...
def write_results_summary(out_dir, responses, means, pca_results,
    ...
    with open(path, "w") as f:
        f.write("# Gray Replication with AI/Human Characters\n")
        ...
        f.write(
            ...
            "Do the factors separate AI from human characters?\n\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Gray replication with AI/human characters"
    )
    ...
    for model_key in models_to_run:
        ...
        print(f"{'='*60}\n")
        set_model(model_key)
        run_gray_characters(args)


def run_gray_characters(args):
    """Run the Gray-with-characters pairwise comparisons."""
    device = get_device()
    ...
    # Output dirs
    ddir = data_dir("behavior", "gray_characters")
    rdir = results_phase_dir("behavior", "gray_characters")
