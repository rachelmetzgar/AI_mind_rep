#!/usr/bin/env python3
"""
Experiment 4, Phase 1a: RSA Report Generation

Generates a self-contained HTML report with figures for RSA-by-dimension
analysis. Supports both chat and base model variants via --model flag.
...
For each RSA variant (combined, experience, agency), generates:
  ...
  - 4-panel RDM overview: model RDM at peak + all 3 human RDMs

Saves figures as PNG/PDF and embeds as base64 in HTML.

Usage:
    python 1a_rsa_report_generator.py --model llama2_13b_chat
    python 1a_rsa_report_generator.py --model llama2_13b_base
...
from utils.report_utils import add_dataset_argument, gray_entities_stimuli_html
...
def load_rsa(tag):
    """Load rsa_results.json from data_dir("gray_simple", "internals", tag)."""
    path = data_dir("gray_simple", "internals", tag) / "rsa_results.json"
    if not path.exists():
        return None
    with open(path) as f:
        return json.load(f)
...
def load_rdm(tag):
    """Load rdm_cosine_per_layer.npz from data_dir("gray_simple", "internals", tag)."""
    path = data_dir("gray_simple", "internals", tag) / "rdm_cosine_per_layer.npz"
    if not path.exists():
        return None
...
def get_human_rdm(rdm_data, variant_name):
    """Get human RDM from npz, handling old-format fallback."""
...
def save_fig(fig, name, fig_dir):
    """Save figure as PNG/PDF and return base64 PNG string."""
...
def fig_rsa_bars(rsa_ns, rsa_ws, variant_name, model_label, fig_dir):
    """1x2 bar chart: without_self (left), with_self (right).
    Gray bars default; colored for p < .05. Peak annotated with arrow."""
    ...
    fig.suptitle(
        ...
        fontsize=13, y=1.06)
    fig.tight_layout()
    return save_fig(fig, f"rsa_bars_{variant_name}", fig_dir)
...
def fig_rdm_pair(rdm_data, rsa_results, variant_name, entity_keys, cond,
                 fig_dir):
    """1x2 heatmap: human RDM (left) vs model RDM at peak layer (right)."""
    vlabel = VARIANT_LABELS[variant_name]
    peak = find_peak(rsa_results)
    if peak["layer"] < 0:
        return None
    ...
    fig.suptitle(
        f"RDM: {vlabel} -- {n} Entities ({cond_label})",
        fontsize=14, y=1.02)
    fig.tight_layout()
    return save_fig(fig, f"rdm_{variant_name}_{cond}", fig_dir)


# ============================================================================
# FIGURE TYPE 3: OVERLAY (line plot)
# ============================================================================

def fig_rsa_overlay(rsa_data, cond_label, n_entities, model_label, fig_dir):
    """All 3 RSA variants overlaid on one line plot."""
    ...
    return save_fig(fig, f"rsa_layerwise_{tag}", fig_dir)
...
def fig_rdm_overview(rdm_data, rsa_data, cond_label, entity_keys,
                     model_label, fig_dir):
    """Model RDM at combined peak + all 3 human RDMs side-by-side."""
    model_rdm = rdm_data["model_rdm"]
    ...
    fig.tight_layout()
    tag = cond_label.lower().replace(" ", "_")
    return save_fig(fig, f"rdm_comparison_{tag}", fig_dir)
...
def html_fig(b64, caption):
    """Walkthrough-style <figure> block."""
    return (f'<figure>\n'
...
def generate_html(model_key, dataset="full_dataset"):
    ...
    if config.IS_CHAT:
        PROMPT_DESC = (
            "Each entity prompt is wrapped in the LLaMA-2 chat template "
            "(<code>[INST]...system prompt...[/INST]</code>) with the default "
            "system prompt. The model sees one user turn: the entity prompt."
        )
    else:
        PROMPT_DESC = (
    ...
    if rsa_ws is None and rsa_ns is None:
        print(f"ERROR: No rsa_results.json found for {model_key}. "
    ...
    # Use figures from the without_self condition dir as the primary figure dir,
    # but we generate per-condition figures into each condition's figures dir
    print("Generating figures...")
    figs = {}  # key -> base64

    # Per-variant bar charts and RDM pairs
    for vname in ["combined", "experience", "agency"]:
        rsa_v_ns = rsa_ns[vname] if rsa_ns else None
        rsa_v_ws = rsa_ws[vname] if rsa_ws else None

        # Bar chart needs a figure dir -- use without_self if available
        bar_fig_dir = str(figures_dir("gray_simple", "internals",
                          "without_self" if rsa_ns else "with_self"))

        # Bar chart (fig7-style)
        figs[f"bars_{vname}"] = fig_rsa_bars(
            rsa_v_ns, rsa_v_ws, vname, MODEL_LABEL, bar_fig_dir)

        # RDM pairs (fig10-style) per condition
        if rdm_ns is not None and rsa_v_ns is not None:
            ns_fig_dir = str(figures_dir("gray_simple", "internals", "without_self"))
            b = fig_rdm_pair(rdm_ns, rsa_v_ns, vname, entity_keys_ns,
                             "without_self", ns_fig_dir)
            if b:
                figs[f"rdm_{vname}_ns"] = b
        if rdm_ws is not None and rsa_v_ws is not None:
            ws_fig_dir = str(figures_dir("gray_simple", "internals", "with_self"))
            b = fig_rdm_pair(rdm_ws, rsa_v_ws, vname, ENTITY_NAMES,
                             "with_self", ws_fig_dir)
            if b:
    ...
    if rsa_ns is not None:
        ns_fig_dir = str(figures_dir("gray_simple", "internals", "without_self"))
        figs["overlay_ns"] = fig_rsa_overlay(
            rsa_ns, "Without Self", 12, MODEL_LABEL, ns_fig_dir)
    if rsa_ws is not None:
        ws_fig_dir = str(figures_dir("gray_simple", "internals", "with_self"))
        figs["overlay_ws"] = fig_rsa_overlay(
            rsa_ws, "With Self", 13, MODEL_LABEL, ws_fig_dir)

    # 4-panel RDM overviews
...
<li><a href="#stimuli">2. Stimuli</a></li>
<li><a href="#combined">3. Combined RSA</a></li>
<li><a href="#experience">4. Experience-Only RSA</a></li>
<li><a href="#agency">5. Agency-Only RSA</a></li>
<li><a href="#overlay">6. Overlay Comparison</a></li>
<li><a href="#comparison">7. Summary Table</a></li>
</ol>
</nav>
...
"""
    ...
    for vname, vlabel, sid in [
        ...
        section_num = {"combined": 3, "experience": 4, "agency": 5}[vname]
    ...
    # ── Overlay section ──
    html += '<h2 id="overlay">6. Overlay Comparison</h2>\n'
    ...
    html += '<h2 id="comparison">7. Summary Table</h2>\n'
    html += '<div class="card">\n'
    ...
    html += (f'\n<p class="small" style="margin-top:2rem;text-align:center">'
             f'Generated by <code>1a_rsa_report_generator.py --model {model_key}</code></p>\n'
             '</body>\n</html>')

    # Write report
    out_dir = results_dir("gray_simple", "internals") / dataset
