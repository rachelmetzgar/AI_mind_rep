...
Usage:
    ...
    python 4_behavior_analysis.py --mode v1 --version labels
    python 4_behavior_analysis.py --mode v2 --version labels

    # Specific strength only:
    python 4_behavior_analysis.py --mode v1 --version labels --strength 16
    python 4_behavior_analysis.py --mode v2 --version balanced_names --strength 8

    # Custom input root:
    python 4_behavior_analysis.py --mode v1 --version labels --input data/labels/intervention_results/V1
...
EXP1_UTILS_DIR = None  # Set after set_version() in main via _init_utils()

# Module-level globals populated by _init_utils()
DEMIR_ALL_HEDGES = None
DEMIR_NOUNS = None
DEMIR_ADJECTIVES = None
DEMIR_ADVERBS = None
DEMIR_VERBS = None
DEMIR_QUANTIFIERS = None
DEMIR_MODALS = None

FUNG_INTERPERSONAL = None
FUNG_REFERENTIAL = None
FUNG_STRUCTURAL = None
FUNG_COGNITIVE = None
FUNG_ALL_23_MARKERS = None

LIWC_NONFLUENCIES = None
LIWC_FILLERS = None
LIWC_DISFLUENCIES = None
TOM_PHRASES = None
POLITE_POSITIVE = None
POLITE_NEGATIVE = None
IMPOLITE = None
LIKE_MARKER = None
...
def _load_util_module(name):
    ...
    if not os.path.exists(fpath):
        raise FileNotFoundError(
            ...
            f"  Ensure set_version() was called and exp1_utils path is correct."
        )
...
def _init_utils():
    """Initialize EXP1_UTILS_DIR and load linguistic marker modules.

    Must be called after set_version() so that cfg.PATHS.exp1_utils is set.
    """
    global EXP1_UTILS_DIR
    global DEMIR_ALL_HEDGES, DEMIR_NOUNS, DEMIR_ADJECTIVES, DEMIR_ADVERBS
    global DEMIR_VERBS, DEMIR_QUANTIFIERS, DEMIR_MODALS
    global FUNG_INTERPERSONAL, FUNG_REFERENTIAL, FUNG_STRUCTURAL
    global FUNG_COGNITIVE, FUNG_ALL_23_MARKERS
    global LIWC_NONFLUENCIES, LIWC_FILLERS, LIWC_DISFLUENCIES
    global TOM_PHRASES, POLITE_POSITIVE, POLITE_NEGATIVE, IMPOLITE, LIKE_MARKER

    EXP1_UTILS_DIR = str(cfg.PATHS.exp1_utils)
...
def main():
    ...
    # Activate data version and load linguistic marker utils
    ...
    if args.input is None:
        ir = cfg.PATHS.intervention_results_v1 if args.mode == "v1" else cfg.PATHS.intervention_results_v2
        args.input = str(ir / args.layer_strategy)
