...
Saves a comprehensive CSV for offline analysis (1e script).

Usage:
    python 1d_degradation_probe_correlation.py --version labels

Env: llama2_env (needs GPU)
Rachel C. Metzgar · Feb 2026
"""

import os, sys, csv, json, re, pickle, collections, argparse
import numpy as np
import pandas as pd
import torch
...
from config import config, set_version, add_version_argument, add_model_argument, set_model, ensure_dir

# ========================== CONFIG ========================== #
# Version-dependent paths (CSV_DIR, PROBE_BASE, OUT_DIR) are resolved
# at runtime via config after set_version() is called in __main__.
...
def load_probe(turn, probe_type, layer):
    ...
    path = config.PATHS.probe_checkpoints / f"turn_{turn}" / probe_type / f"human_ai_probe_at_layer_{layer}.pth"
    if not path.exists():
        return None
    probe = LinearProbeClassification(
        device="cpu", probe_class=1, input_dim=config.INPUT_DIM, logistic=True,
    )
...
def get_peak_layers():
    ...
    for turn in TURNS:
        for pt in PROBE_TYPES:
            pkl_path = config.PATHS.probe_checkpoints / f"turn_{turn}" / pt / "accuracy_summary.pkl"
            if pkl_path.exists():
...
def main():
    ...
    # Save
    out_dir = ensure_dir(config.RESULTS.degradation)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Degradation-Probe Correlation Analysis (GPU)"
    )
