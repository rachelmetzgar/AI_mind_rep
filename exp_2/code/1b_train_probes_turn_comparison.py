...
Usage:
    python code/pipeline/1b_train_probes_turn_comparison.py --version labels --turn_index 2
    python code/pipeline/1b_train_probes_turn_comparison.py --version labels --turn_index -1

Env: llama2_env (needs GPU)
Rachel C. Metzgar · Feb 2026
"""

import os, sys, argparse, pickle, numpy as np, torch, torch.nn as nn
...
from config import config, set_version, add_version_argument, add_model_argument, set_model
...
def run_probe_train(model, tokenizer, control_probe, out_subdir, turn_index, probe_dir):
    ...
    for layer_num in range(n_layers):
        ...
        probe = LinearProbeClassification(
            device=DEVICE, probe_class=1, input_dim=config.INPUT_DIM, logistic=config.TRAINING.logistic,
        )
...
if __name__ == "__main__":
    ...
    probe_dir = str(config.PATHS.probe_checkpoints / turn_label)
