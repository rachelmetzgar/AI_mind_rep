...
Output: exp_2/results/comparisons/probe_training/alt_tokens/weather_suffix_investigation.html
...
OUTPUT_PATH = f'{BASE}/results/comparisons/probe_training/alt_tokens/weather_suffix_investigation.html'
...
def run_c1_analysis(version='labels', n_samples=50, cache_dir=None):
    ...
    if cache_dir is None:
        cache_dir = f'{BASE}/data/{version}/weather_investigation'
...
def load_probe_accuracies(version, turn=5):
    ...
    path = f'{BASE}/data/{version}/probe_checkpoints/turn_{turn}/metacognitive/accuracy_summary.pkl'
    try:
    ...
    path = f'{BASE}/data/{version}/probe_checkpoints/turn_{turn}/operational/accuracy_summary.pkl'
    try:
        with open(path, 'rb') as f:
            d = pickle.load(f)
...
def main():
    ...
    if args.run_c1:
        ...
        # Try to load cached
        cache_path = f'{BASE}/data/{args.version}/weather_investigation/cosine_similarities.npz'
        if os.path.exists(cache_path):
