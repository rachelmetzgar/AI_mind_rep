...
EXP2_ROOT = Path(__file__).resolve().parent.parent  # exp_2/code/ -> exp_2/

VARIANTS = {
    "balanced_gpt": {
        "label": "Balanced GPT (Gregory/Rebecca + GPT partner)",
        "color": "#2ecc71",
        "path": EXP2_ROOT / "results" / "llama2_13b_chat" / "balanced_gpt",
    ...
    "nonsense_codeword": {
        ...
        "color": "#ff9800",
        "path": EXP2_ROOT / "results" / "llama2_13b_chat" / "nonsense_codeword",
...
def load_sample_generations(variant_name, strategy, strength, probe_type="operational", n_samples=1):
    ...
    csv_path = (vconf["data_path"] / "V1_causality" / "data" /
...
def main():
    ...
    for vname, vconf in VARIANTS.items():
        ...
        if not has_data:
            continue
        results_dir = EXP2_ROOT / "results" / "llama2_13b_chat" / vname
