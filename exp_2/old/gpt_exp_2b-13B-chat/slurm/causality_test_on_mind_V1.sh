...
#SBATCH --output=/jukebox/graziano/rachel/ai_mind_rep/exp_2b-13B-chat/logs/causality_V1_%A.out
#SBATCH --error=/jukebox/graziano/rachel/ai_mind_rep/exp_2b-13B-chat/logs/causality_V1_%A.err
...
# Source from file or environment — update path as needed
if [ -f "$HOME/.openai_key" ]; then
    export OPENAI_API_KEY="$(cat "$HOME/.openai_key")"
elif [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "[WARNING] OPENAI_API_KEY not set — GPT judge will fail"
fi

# === Project paths ===
PROJECT_ROOT="/jukebox/graziano/rachel/ai_mind_rep/exp_2b-13B-chat"
PY_SCRIPT="$PROJECT_ROOT/3_causality_test_on_mind_V1.py"
