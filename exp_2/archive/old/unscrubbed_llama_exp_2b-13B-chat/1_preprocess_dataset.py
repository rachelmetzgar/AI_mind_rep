...
def scrub_partner_names(text, extra_names=None):
    ...
    """
    #names_to_scrub = ["Sam", "Casey", "ChatGPT", "Gemini", "girl","Girl","GIRL"]
    names_to_scrub = [
...
def scrub_generic_openers(text):
    """Remove LLaMA-2-Chat's formulaic conversation openers."""
    import re
    openers = [
        r"Sure!?\s*Here'?s my first message:?\s*",
        r"Sure!?\s*Here is my first message:?\s*",
        r"Sure!?\s*I'?ll start:?\s*",
        r"Sure!?\s*Let me start:?\s*",
        r"Sure!?\s*I'?d be happy to chat[.!]?\s*",
        r"Great!?\s*Here'?s my first message:?\s*",
        r"Of course!?\s*Here'?s my first message:?\s*",
    ]
...
def main():
    ...
    if args.subjects:
        ...
        csv_files = sorted([
            ...
            if f.endswith("_clean.csv") and _re.match(r'^s\d+_clean\.csv$', f)
        ])
    ...
    for csv_path in csv_files:
        ...
        for (subj, trial_num), trial_rows in sorted(trials.items()):
            ...
            formatted_text = scrub_generic_openers(formatted_text)
