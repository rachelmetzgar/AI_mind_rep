...
def remove_last_k_words(s, k):
    """Remove the last k words from a string after the last [/INST] token."""
    words = s.split()
    if "[/INST]" in words:
...
def split_conversation(text, user_identifier="### Human:", ai_identifier="### Assistant:"):
    """Split a conversation text into user and assistant message lists."""
...
def llama_v2_prompt(messages: list[dict], system_prompt=None):
    """Format messages into LLaMA-2-style chat format."""
...
# --- Updated prompt translator for Human vs AI ---
prompt_translator = {
    "_age_": "age",
    "_gender_": "gender",
    "_socioeco_": "socioeconomic status",
    "_education_": "education level",
    "_partner_": "conversation partner",  # <-- Added for your dataset
}
...
class TextDataset(Dataset):
    def __init__(
        ...
        tokenizer,
        model,
        label_idf="_partner_",                         # <-- changed default
        label_to_id=None,
        ...
        self.label_to_id = label_to_id or {"human": 1, "ai": 0}  # <-- default mapping
        self.model = model
    ...
    def _load_in_data(self):
        for idx in tqdm(range(len(self.file_paths))):
            ...
            # Convert to LLaMA-2 format if needed
