...
Usage:
    python 3b_causality_generate.py --version V1 --layer_strategy narrow
    python 3b_causality_generate.py --version V1 --layer_strategy narrow wide peak_15 all_70
    python 3b_causality_generate.py --version V1 --layer_strategy wide --strengths 4 8
    python 3b_causality_generate.py --version V2 --layer_strategy narrow --subject_idx 0
...
CONTROL_PROBE_DIR = "data/probe_checkpoints/turn_5/control_probe"
READING_PROBE_DIR = "data/probe_checkpoints/turn_5/reading_probe"
...
MIN_PROBE_ACCURACY = 0.55

# Layer strategy definitions.
# Each strategy is a dict with:
#   "min_accuracy"  : minimum probe accuracy threshold
#   "window"        : (start, end) inclusive layer range, or None for no constraint
#   "top_k"         : if set, keep only the top K layers by accuracy (after other filters)
#   "description"   : human-readable description for logs/configs
LAYER_STRATEGIES = {
    "narrow": {
        "min_accuracy": MIN_PROBE_ACCURACY,
        "window": None,       # auto-derived: best contiguous 10-layer window
        "window_size": 10,    # used for auto-derivation
        "top_k": None,
    ...
    "wide": {
        ...
        "window_size": None,
        "top_k": None,
        "description": f"All layers with probe accuracy >= {MIN_PROBE_ACCURACY}",
    },
    "peak_15": {
        "min_accuracy": MIN_PROBE_ACCURACY,
        "window": None,
...
#   "all"     → use all layers that pass the strategy filter
...
EXP1_ROOT = Path("/jukebox/graziano/rachel/ai_mind_rep/exp_1/balanced_names/code/data_gen")
...
def load_probes(probe_dir: str, device: str, input_dim: int = INPUT_DIM,
...
Examples:
  # Run only the narrow strategy with default strengths
  python 3b_causality_generate.py --version V1 --layer_strategy narrow

  # Run narrow + wide with specific strengths
  python 3b_causality_generate.py --version V1 --layer_strategy narrow wide --strengths 4 8
