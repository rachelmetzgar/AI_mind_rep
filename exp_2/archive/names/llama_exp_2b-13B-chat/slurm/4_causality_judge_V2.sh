...
#SBATCH --time=4:00:00
#SBATCH --array=0-99
#SBATCH --output=/jukebox/graziano/rachel/ai_mind_rep/exp_2/llama_exp_2b-13B-chat/logs/causality_judge_v2/judge_V2_%A_%a.out
#SBATCH --error=/jukebox/graziano/rachel/ai_mind_rep/exp_2/llama_exp_2b-13B-chat/logs/causality_judge_v2/judge_V2_%A_%a.err

# ---------------------------------------------------------------------------
# V2 Judge: parallelized by subject × strength.
# 2D array: 50 subjects × N_STRENGTHS strengths
#   subject_idx = TASK_ID / N_STRENGTHS
#   strength    = STRENGTHS[TASK_ID % N_STRENGTHS]
# Each job judges BOTH probe types for one subject at one strength.
# No GPU needed.
#
# Update STRENGTHS and --array range to match what was generated.
# E.g., 50 subjects × 2 strengths = --array=0-99
#
# Submit after V2 generation completes:
#   sbatch 4_causality_judge_V2.sh
# ---------------------------------------------------------------------------
...
# === Project paths ===
PROJECT_ROOT="/jukebox/graziano/rachel/ai_mind_rep/exp_2/llama_exp_2b-13B-chat"
...
for PROBE_TYPE in control_probes reading_probes_peak; do
    RESULT_DIR="$PROJECT_ROOT/data/intervention_results/V2/${STRATEGY}/${PROBE_TYPE}/is_${N}"
    if [ -d "$RESULT_DIR/per_subject" ] && [ -f "$RESULT_DIR/per_subject/${SUBJECT_ID}.csv" ]; then
        ...
        python "$PY_SCRIPT" --version V2 --layer_strategy "$STRATEGY" --result_dir "$RESULT_DIR" --subject "$SUBJECT_ID"
    else
