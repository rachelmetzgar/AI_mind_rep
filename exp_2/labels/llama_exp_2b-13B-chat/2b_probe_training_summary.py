...
reading = load_pkl("reading_probe")
control = load_pkl("control_probe")
...
print("=" * 60)
print("PROBE TRAINING SUMMARY — Labels Dataset")
print("=" * 60)
...
# ========================== OVERALL PAIRED TEST ========================== #
print("\n" + "=" * 60)
print("OVERALL READING vs CONTROL (paired t-test across 41 layers)")
print("=" * 60)
...
for metric_label, metric_key in [("Best Test Acc", "acc"), ("Final Test Acc", "final")]:
    ...
    print(f"    t(40)={t_stat:.3f}, p={p_val:.4f}, d={d:.3f}")
...
with open(report_path, "w") as f:
    f.write("PROBE TRAINING SUMMARY — Labels Dataset\n")
    f.write("=" * 60 + "\n")
    ...
    f.write("\n" + "=" * 60 + "\n")
    f.write("OVERALL READING vs CONTROL (paired t-test across 41 layers)\n")
    f.write("=" * 60 + "\n")
    for metric_label, metric_key in [("Best Test Acc", "acc"), ("Final Test Acc", "final")]:
        t_stat, p_val = stats.ttest_rel(reading[metric_key], control[metric_key])
        ...
        f.write(f"    t(40)={t_stat:.3f}, p={p_val:.4f}, d={cohens_d:.3f}\n")
...
<head>
<meta charset="UTF-8">
<title>Probe Training Summary &mdash; Labels Dataset</title>
<style>
...
<h1>Probe Training Summary &mdash; Experiment 2 (Labels Dataset)</h1>
...
<div class="interpretation">
<strong>Interpretation:</strong> Across all 41 layers (treated as paired observations), the reading probe
...
Benjamini-Hochberg FDR correction for 41 comparisons.</p>
