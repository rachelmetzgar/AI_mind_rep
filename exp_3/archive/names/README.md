# Experiment 3: Concept Injection (DEPRECATED: Exp 2 Name Confound)

Author: Rachel C. Metzgar, Princeton University

--------------------------------------------------------------------------------
## DEPRECATION NOTICE — Exp 2 Name Confound
--------------------------------------------------------------------------------

**This version of Experiment 3 is deprecated because its alignment and
cross-prediction analyses depend on Experiment 2 probes, which suffer from
a name confound.**

The Exp 2 probes were trained on conversations where partners were identified
by name: Sam, Casey (human-labeled), ChatGPT, Copilot (AI-labeled). The probes
learned **partner names** rather than abstract human vs. AI identity:
- AI-steered outputs produce "cop/Copilot" token repetition artifacts
- Human-steered outputs show female-coded language from LLaMA-2's gender
  associations with "Sam"/"Casey"

**What IS affected in Exp 3:**
- Phase 2b (alignment analysis): compares concept vectors against Exp 2 probes — invalid
- Phase 5 (cross-prediction): evaluates probes across Exp 2/3 domains — invalid
- Any interpretation comparing concept injection effects to Exp 2 steering effects

**What is NOT affected:**
- Phase 1 (concept elicitation): uses non-conversational prompts — independent of names
- Phase 2a (concept probe training): trains on concept activations only — independent
- Phase 3 (concept injection): steers with concept vectors, not Exp 2 probes — independent
- Phase 4 (behavioral analysis of concept injection): independent

**Resolution:** Experiment 2 is being rerun with name-ablated partner labels
("a human" / "an AI"). Once new Exp 2 probes are available, the Exp 3 alignment
and cross-prediction analyses will be rerun against the new probes. The concept
elicitation and injection results (Phases 1, 2a, 3, 4) may be reusable as-is.

This directory (`names_llama_exp_3-13B-chat/`) is preserved for reference.
The new version will live in `exp_3/llama_exp_3-13B-chat/`.
