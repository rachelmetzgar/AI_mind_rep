#!/usr/bin/env python3
"""
Experiment 3, Phase 2: Train Concept Probes
...
For a SINGLE concept dimension (specified by --dim_id):
    ...
    2. Optionally computes quick alignment with Exp 2b conversational probes
       (the authoritative alignment analysis is in 2b_alignment_analysis.py)

Designed to run in parallel via SLURM array jobs (one job per dimension).

Inputs:
    data/concept_activations/contrasts/{dim_name}/concept_activations.npz
    data/concept_activations/contrasts/{dim_name}/concept_vector_per_layer.npz
...
# Activations live under contrasts/ in the new pipeline
...
# Experiment 2b probes (for quick sanity-check alignment)
EXP2B_ROOT = (
