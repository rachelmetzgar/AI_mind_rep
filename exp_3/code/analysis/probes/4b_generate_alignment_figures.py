...
Usage:
    python 4b_generate_alignment_figures.py --version labels --mode contrast
    python 4b_generate_alignment_figures.py --version balanced_names --mode standalone
    python 4b_generate_alignment_figures.py --version labels --mode both
...
def load_stats(mode):
    """Load statistics JSON for the specified mode (version-aware)."""
    if mode == "contrast":
        base = get_version_results_dir(config.RESULTS.root / "concept_probe_alignment")
        stats_json = str(base / "summaries" / "alignment_stats.json")
    else:  # standalone
        base = get_version_results_dir(config.RESULTS.root / "standalone_alignment")
        stats_json = str(base / "summaries" / "standalone_alignment_stats.json")

    if not os.path.isfile(stats_json):
        print(f"ERROR: Stats file not found: {stats_json}")
        print(f"Run 4a_compute_alignment_stats.py --version <version> --mode {mode} first!")
        return None
...
def generate_figures_for_mode(mode):
    ...
    # Set up output directory (version-aware)
    if mode == "contrast":
        out_dir = str(get_version_results_dir(config.RESULTS.root / "concept_probe_alignment") / "figures")
    else:
        out_dir = str(get_version_results_dir(config.RESULTS.root / "standalone_alignment") / "figures")
