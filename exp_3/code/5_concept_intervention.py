#!/usr/bin/env python3
"""
Experiment 3, Phase 5: Concept Injection into Conversations — GENERATION ONLY.

Injects concept vectors (from per-dimension concept probes trained in Phase 3)
...
Usage:
    python 5_concept_intervention.py --mode V1 --dim_id 7
    python 5_concept_intervention.py --mode V1 --dim_id 7 --strengths 4 8
    python 5_concept_intervention.py --mode V2 --dim_id 7 --subject_idx 0
    python 5_concept_intervention.py --mode V1 --dim_ids 1 5 7 11 13
...
CONCEPT_PROBE_ROOT = str(config.RESULTS.concept_probes_data)

INPUT_DIM = config.INPUT_DIM
DEVICE = config.get_device()
TORCH_DEVICE = DEVICE

DEFAULT_STRENGTHS = config.TRAINING.default_strengths
MIN_PROBE_ACCURACY = config.TRAINING.min_probe_accuracy

# --- V1 config ---
CAUSAL_QUESTION_PATH = str(config.PATHS.causality_questions)
RESULT_DIR_V1 = str(config.RESULTS.interventions_v1)
V1_MAX_NEW_TOKENS = config.GEN_V1.max_new_tokens
V1_GEN_TEMPERATURE = config.GEN_V1.temperature
V1_GEN_TOP_P = config.GEN_V1.top_p
V1_GEN_DO_SAMPLE = config.GEN_V1.do_sample

# --- V2 config ---
RESULT_DIR_V2 = str(config.RESULTS.interventions_v2)
EXP1_ROOT = config.EXP1_ROOT
PROMPTS_DIR = config.PATHS.exp1_prompts
CONFIG_DIR = config.PATHS.exp1_configs
V2_MAX_NEW_TOKENS = config.GEN_V2.max_new_tokens
V2_GEN_TEMPERATURE = config.GEN_V2.temperature
V2_GEN_TOP_P = config.GEN_V2.top_p
PAIRS_TOTAL = config.GEN_V2.pairs_total
HISTORY_PAIRS = config.GEN_V2.history_pairs
CONDITIONS = config.GEN_V2.conditions
SYSTEM_PROMPT = config.GEN_V2.system_prompt
LOG_DIR = config.RESULTS.logs
