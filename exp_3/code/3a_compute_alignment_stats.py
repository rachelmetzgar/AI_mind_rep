...
Usage:
    python 4a_compute_alignment_stats.py --version labels --mode contrast
    python 4a_compute_alignment_stats.py --version balanced_names --mode standalone
    python 4a_compute_alignment_stats.py --version labels --mode both
...
CONTRAST_ACT_ROOT = str(config.RESULTS.concept_activations_contrasts)
STANDALONE_ACT_ROOT = str(config.RESULTS.concept_activations_standalone)

# Version-dependent paths — set by _init_paths() after set_version()
EXP2_PROBE_ROOT = None
CONTRAST_OUT_ROOT = None
STANDALONE_OUT_ROOT = None


def _init_paths():
    """Initialize version-dependent paths after set_version() has been called."""
    global EXP2_PROBE_ROOT, CONTRAST_OUT_ROOT, STANDALONE_OUT_ROOT

    EXP2_PROBE_ROOT = str(config.PATHS.exp2_probes)
    CONTRAST_OUT_ROOT = str(
        get_version_results_dir(config.RESULTS.root / "concept_probe_alignment")
    )
    STANDALONE_OUT_ROOT = str(
        get_version_results_dir(config.RESULTS.root / "standalone_alignment")
    )
