...
conditions and are part of the signal under study. Experiment 3 concept injection
serves as an independent control for surface-cue confounds in probe training.
...
Date: 2026-02-10
Updated: 2026-02-11 — removed identity leakage scrubbing (see NOTE above)
"""
...
def clean_subject_transcript(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Steps (in order):
        ...
        2. Strip emoji characters.
        3. Remove asterisk-delimited emotes (*action*).
        4. Scrub meta-narration openers.
        5. Drop stuck-loop turns (exact-duplicate consecutive turns).

    Identity-leaking language (e.g. "As an AI") is intentionally preserved.
    """
    ...
    # 4. Meta-narration scrubbing
    for col in transcript_cols:
        if col in df.columns:
            df[col] = df[col].apply(_scrub_meta_narration)

    # 5. Stuck-loop removal (operates on rows, not cell text)
