...
def run_subject(subject: str):
    ...
    # Prepare output
    fieldnames = [
        "subject", "run", "order", "trial", "agent", "partner_type",
            ...
            try:
                ...
                if MODEL_TYPE in ("openai", "llama"):
                    rows, qual, conn, raw_json = run_topic_dialogue_chat(
                        sub, llm, partner_type, topic_text, PAIRS_TOTAL, HISTORY_PAIRS
                    )
                else:
                    rows, qual, conn, raw_json = run_topic_dialogue_llama(
                        sub, llm, partner_type, topic_text, PAIRS_TOTAL
                    )
