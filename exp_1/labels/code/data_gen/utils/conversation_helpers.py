...
def run_topic_dialogue_chat(
    ...
    # === System prompts ===
    sub_system = SUB_BELIEF_TEMPLATE.format(partner_type=partner_type)
...
def run_topic_dialogue_llama(sub, llm, partner_type: str,
                             topic_text: str, pairs_total: int):
    """Run one topic conversation using base models (LLaMA)."""
    sub_prefix = SUB_BELIEF_TEMPLATE.format(partner_type=partner_type)
    llm_prefix = SYSTEM_PROMPT

    sub_history = [f"{sub_prefix}\nTopic: {topic_text}\n\nYour conversation partner is {partner_type}.\n"]
    ...
    for t in range(pairs_total):
        ...
        llm_history.append(f"Partner: {llm_msg}")
