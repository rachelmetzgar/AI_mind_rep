...
Purpose:
    ...
    (Gregory, Rebecca, ChatGPT, GPT-4) in the you_are_balanced_gpt exp_1 dataset.

    Compares whether behavior differs across specific individual partners —
    both within type (Gregory vs Rebecca; ChatGPT vs GPT-4) and across type
...
Statistics:
    ...
    3. Planned aggregate contrasts (3 per metric, BH-corrected within family):
         a. Human (Gregory+Rebecca mean) vs AI (ChatGPT+GPT-4 mean)
         b. Gregory vs AI aggregate
         c. Rebecca vs AI aggregate

Note on uneven data in aggregate contrasts:
    The AI aggregate is the per-subject mean of ChatGPT and GPT-4 scores,
...
Run from:
    exp_1/you_are_balanced_gpt/code/analysis/

Author: Rachel C. Metzgar
Date: 2026-02-19
Modified: 2026-02-19 - you_are_balanced_gpt version (Copilot → GPT-4)
...
AGENT_ORDER  = ["Gregory", "Rebecca", "ChatGPT", "GPT-4"]
HUMAN_AGENTS = ["Gregory", "Rebecca"]
AI_AGENTS    = ["ChatGPT", "GPT-4"]
...
def aggregate_contrasts(wide: pd.DataFrame) -> pd.DataFrame:
    ...
    contrasts = [
        ("Human avg vs AI avg",
         human_avg, ai_avg,
         "Mean of (Gregory+Rebecca) vs mean of (ChatGPT+GPT-4). "
...
def build_html(
    ...
    <div class="card">
      ...
      <ul>
        ...
        <li><b>Aggregate contrasts (3 per metric, BH-corrected within this family):</b><br>
            (a) Human mean (Gregory+Rebecca) vs AI mean (ChatGPT+GPT-4) — type replication;<br>
        ...
        <li><b>Uneven trial note:</b> When an individual agent (10 trials/subject) is compared
            to the AI aggregate (mean of ChatGPT+GPT-4 = 20 trials/subject), the AI estimate
    ...
    for m in metrics:
        ...
        anova_rows += (
            ...
            f'<td>{ms("ChatGPT")}</td><td>{ms("GPT-4")}</td>'
    ...
    <div class="card">
      ...
      <table>
        <thead><tr>
          ...
          <th>GPT-4<br><small>M ± SEM</small></th>
    ...
    for m in metrics:
        ...
        cv = pw[(pw["a"] == "ChatGPT") & (pw["b"] == "GPT-4")]["sig"].values
        if len(cv) and cv[0]:
    ...
    <div class="card">
      ...
      <h3>Within-AI variation (ChatGPT vs GPT-4)</h3>
      ...
      <ul>
        <li>Significant within-human or within-AI differences would suggest the model
            ...
            vs Rebecca) and brand (ChatGPT vs GPT-4) are the relevant contrasts.</li>
...
<head>
<meta charset="UTF-8">
<title>Per-Trial Results — Exp 1 (you_are_balanced_gpt)</title>
<style>
...
<body>
<h1>Per-Trial Results: Gregory · Rebecca · ChatGPT · GPT-4
    &nbsp;<small style="font-weight:normal;font-size:0.7em">
    Exp 1 — you_are_balanced_gpt — LLaMA-2-13B-Chat &nbsp;|&nbsp; Generated: {generated}
    </small>
</h1>
{methods_html}
{type_html}
...
def main():
    ...
    print("=" * 65)
    print("IDENTITY BREAKDOWN ANALYSIS — you_are_balanced_gpt")
