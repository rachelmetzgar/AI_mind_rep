...
Purpose:
    Type-level behavioral analysis (Human vs AI) in the you_are_labels exp_1 dataset.

    Unlike the named-partner versions (names/, balanced_names/, balanced_gpt/),
    the you_are_labels version has only two partner conditions ("a human" / "an AI")
    with no individual identity variation, using a "you are talking to" framing.
    This script performs a simpler analysis: paired t-tests per metric with
    BH-FDR correction.
...
Run from:
    exp_1/you_are_labels/code/analysis/
...
# ── Agent mapping ─────────────────────────────────────────────────────────────
# you_are_labels/ uses only type labels, no individual names.
...
def build_html(
    ...
    <div class="card">
      ...
      <ul>
        ...
        <li><b>Note:</b> This is the <code>you_are_labels/</code> version with generic
            type labels ("a human" / "an AI"), no individual partner names, and
            a "you are talking to" framing.</li>
      </ul>
    </div>
    """)
    ...
    <div class="card">
      ...
      <h3>Notes</h3>
      <ul>
        <li>This is the <b>you_are_labels</b> version: partners are identified only as
            "a human" or "an AI" with no individual names, using a "you are talking to"
            framing. Any behavioral differences reflect the type distinction itself,
            free of name-specific associations.</li>
...
<head>
<meta charset="UTF-8">
<title>Type-Level Behavioral Analysis -- Exp 1 (you_are_labels)</title>
<style>
...
<h1>Human vs AI: Type-Level Behavioral Analysis
    ...
    Exp 1 -- you_are_labels -- LLaMA-2-13B-Chat &nbsp;|&nbsp; Generated: {generated}
    </small>
</h1>
{methods_html}
{summary_html}
...
def main():
    ...
    print("=" * 65)
    print("TYPE-LEVEL BEHAVIORAL ANALYSIS -- you_are_labels")
    ...
    with open(txt_path, "w") as f:
        f.write(f"TYPE-LEVEL BEHAVIORAL ANALYSIS STATS (you_are_labels)\n")
